...
### Every licensed asset gets its metadata read before it lands

**Read the provenance of every image that enters this repo, before it is
committed, and `scripts/audit.py` now reads it too.** Decided 2026-09-17,
after two licensed candidates for the We Fix It All render were both caught
being AI-generated by their own embedded metadata.

Rule 9 bans AI imagery, and the reason is the firm's own argument: a site
that tells clients real beats stock has to live by it. Until now that rule
had no mechanism, so it was enforced by somebody choosing to look. Twice in
one afternoon what caught an AI asset was exactly that, which is the shape of
rule that rule 8 says gets violated.

**What the two rejected candidates carried**, both licensed from Adobe Stock:

```
AdobeStock_1060063701   IPTC DigitalSourceType = trainedAlgorithmicMedia
  (top-down)            plus a public Adobe C2PA manifest
AdobeStock_746591791    xmp:CreatorTool = "OkiDokiBot AI Art Generator"
  (side profile)        no DigitalSourceType at all
```

**Two different tells in two different tags, which is why the check reads
both and trusts neither alone.** Either file would have passed a check
written against the other. The first is Adobe's own C2PA label, using the
standard IPTC code for pixels a generative model made. The second declares
no source type and names the generator in the tool field instead.
...
Removing it to make a file pass is a lie about the asset and a licence
breach, and the check's own failure text says so, because the shortcut is
one command and it would occur to anybody in a hurry.

**The limit is real and the report states it.** The check reads labels. It
cannot read pixels, so an AI image whose metadata was stripped passes it, and
a clean result means the file makes no AI claim rather than that a person
made it. The judgement at purchase time is still the control that matters;
the check is the floor under it. What is deliberately **not** a failure:
`digitalCapture` is a camera, and `digitalArt` and `algorithmicMedia` are a
person at a workstation, which is exactly what the real render we are waiting
for will be. Failing those would ban the asset the check exists to let
through.
...
| `scripts/prepare-hero-photo.py` | Turns the client's photograph of a wrecked customer vehicle into the home hero. Reads provenance through the audit's own reader and refuses a flagged file, crops on a measured scan of where the vehicle actually is, runs the plate-detection check even though no plate is visible, strips APP segments structurally by marker walk, asserts the emitted file decodes to the 1200x800 contract, and never emits a file larger than its source. **It imports its helpers from `prepare-repair-photos.py` rather than copying them**, so the APP strip and the detail metric are one implementation. Prints every number it used. |
